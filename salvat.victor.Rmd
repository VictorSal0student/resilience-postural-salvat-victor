...
**Falls** represent a major **public health concern** affecting approximately **one-third of adults aged 65+ years** annually (World Health Organization, 2021), with consequences ranging from physical injuries to loss of autonomy. Among physiological factors contributing to fall risk, **postural control deterioration** plays a central role (Horak, 2006). Traditional balance assessment methods such as clinical scales and static posturography provide valuable but limited insights, often failing to capture **dynamic complexity of real-world perturbations** (Mancini & Horak, 2010).

**Postural resilience**, defined as the ability to recover stability following unexpected perturbations, has emerged as a **critical marker of functional balance capacity** in aging populations (Bruijn et al., 2013). **Time-Delay Embedding (TDE)** offers a mathematical framework derived from dynamical systems theory that reconstructs the underlying **attractor governing postural control** from univariate time series measurements. The TDE approach, formalized by **Takens' embedding theorem** (Takens, 1981), enables **state-space reconstruction** from a single measured variable by creating delayed copies of the signal, revealing geometric structure of system dynamics.

Two critical parameters govern reconstruction quality: **time delay (τ)** determining temporal spacing between embedded dimensions, and **embedding dimension (dim)** specifying reconstructed space dimensionality. Optimal parameter selection (Fraser & Swinney, 1986; Kennel et al., 1992) ensures **accurate representation** while avoiding spurious correlations or insufficient dimensional representation.
...
The present report documents pipeline development and validation on **seven participants (five young adults, two aging adults)**, and synchronization testing on two technical validation sessions.
...
The experimental protocol employed auditory perturbations designed to elicit postural recovery responses. Participants walked **overground** in a rectangular path (**8 × 2.5 m**) following a **clockwise direction** at self-selected comfortable pace. Two types of unexpected auditory perturbations were delivered at randomized intervals: **slow perturbations** consisting of low-frequency beeps (gradual tempo decrease), and **fast perturbations** involving high-frequency beeps (abrupt tempo increase). Each participant completed a single recording session comprising approximately 8-10 perturbations of each type distributed across a 6-minute acquisition period.

Participants were instructed to maintain **natural walking pace**, avoid anticipatory adjustments, and respond naturally to auditory cues. Visual feedback was minimized by having participants focus on a **wall-mounted target** positioned at eye level. No practice trials were provided to preserve **perturbation unexpectedness**.

**The TDE analysis pipeline was implemented in Python, adapted from the original MATLAB methodology developed by Antoine Dufourneau (Dufourneau, 2025).**
...
**Stage 1 - Empirical Mode Decomposition (EMD):** The raw 400 Hz signal was decomposed into **intrinsic mode functions (IMFs)** representing oscillatory components at different temporal scales. EMD **adaptively separates signals** based on local extrema without predetermined basis functions. The **two highest-frequency IMFs** (measurement noise, physiological tremor) were discarded; remaining IMFs were summed to reconstruct a **denoised signal**. This preprocessing step improves **signal stationarity**, a critical prerequisite for reliable state-space reconstruction, by removing non-stationary high-frequency artifacts that could distort attractor geometry.
...
**Embedding dimension (dim)** was determined using **False Nearest Neighbors (FNN)** method developed by Kennel et al. (1992). FNN identifies the **minimum dimension required** to unfold the attractor by detecting **spurious nearest neighbors** from insufficient dimensionality. Optimal dim corresponds to the dimension where **false neighbor percentage drops below 1%**.

For participant **004 (validation case)**, AMI analysis yielded **optimal τ = 19 samples (0.19 seconds at 100 Hz)**. FNN analysis indicated **optimal dim = 5**. These parameters were applied to **all participants** assuming similar postural control dynamics across healthy adults.

### State-Space Reconstruction and Metrics

Following parameter optimization, the preprocessed displacement signal **X(t)** was transformed into **five-dimensional state-space** using delay embedding vectors: **X(t) = [X(t), X(t-τ), X(t-2τ), X(t-3τ), X(t-4τ)]**. This reconstruction preserves **topological properties** of the original system dynamics (Takens' theorem), enabling **quantitative stability and recovery analysis**.
...
**Recovery Time:** Duration required for the state-space trajectory to **return within baseline stability zones**, defined as **mean ± k·SD of Euclidean distances** in the reconstructed state-space during unperturbed walking (before first perturbation), with threshold zones **T₁ (mean + 1SD), T₂ (mean + 2SD), and T₃ (mean + 3SD)**.
...
**False Nearest Neighbors** analysis showed rapid decrease in false neighbor percentage from dimensions 1 to 4, falling below **1% threshold at dim = 5**. Further dimension increases yielded negligible improvement, confirming **dim = 5 as optimal**. This **five-dimensional representation** captures essential postural control dynamics without over-embedding.

```{r tde-params, fig.cap="**Figure 1. TDE Parameter Optimization for Participant 004.** Left: Average Mutual Information vs time delay with optimal τ=19 (red). Right: False Nearest Neighbors percentage vs embedding dimension with optimal dim=5 at **1% threshold** (orange dashed line).", out.width="100%"}
knitr::include_graphics("results/figures/004_tde_parameters.png")
```

Optimized parameters **(τ=19, dim=5)** were applied **uniformly across all participants**, supported by literature demonstrating **consistent characteristic timescales and dimensionality** across healthy adults.

## Perturbation Detection Validation

Detection of perturbation onset was performed automatically from the analog audio signal (beeps) using the BeatMove auditory cueing system. Inter-beep intervals were computed and compared to baseline cadence (mean of first 10 beeps). Perturbations were detected when 3 consecutive intervals deviated beyond ±15% tolerance bounds.

```{r perturbation-detection, fig.cap="**Figure 2. Perturbation Detection Validation (P004).** Gray points = inter-beep intervals. Black dashed line = baseline (0.536s). Red dotted lines = tolerance thresholds (±15%). Blue marker = Slow perturbation onset (173.5s, interval 0.72s). Red marker = Fast perturbation onset (354.6s, interval 0.43s).", out.width="100%"}
knitr::include_graphics("results/figures/004_perturbation_detection.png")
```

For participant 004, the algorithm correctly identified both perturbations: **Slow at 173.54s** (cadence decreased to 75% baseline) and **Fast at 354.61s** (cadence increased to 125% baseline). This automated detection enables systematic analysis across all participants without manual annotation.
...
```{r recovery-curves, fig.cap="**Figure 3. Recovery Dynamics for Slow and Fast Perturbations (P004).** Upper: slow perturbation (peak **26.42 mm**, instant recovery). Lower: fast perturbation (peak **37.15 mm**, recovery **1.22 s**). Black line = distance from reference, colored lines = stability thresholds (T1/T2/T3), red star = perturbation onset, green star = recovery.", out.width="100%"}
knitr::include_graphics("results/figures/004_recovery_curves.png")
```

**Slow perturbations** elicited moderate **peak deviations (26.42 mm)** with **near-instantaneous recovery (0 seconds)**, suggesting **anticipatory or concurrent compensatory adjustments** during gradual perturbation development enabling continuous stability maintenance.

**Fast perturbations** produced larger **peaks (37.15 mm)** requiring measurable **recovery periods (1.22 seconds)**. Increased peak deviation reflects **limited anticipatory compensation capacity** during abrupt perturbations, forcing larger excursions from baseline. Subsequent recovery captures **reactive postural adjustments**, providing quantitative resilience measure.

### State-Space 3D Visualization
Three-dimensional state-space reconstruction reveals the attractor structure with local stability ellipses computed from baseline variability in the Normal-Binormal plane.

```{r fig-torus, echo=FALSE, out.width="100%", fig.cap="**Figure 4.** State-space 3D visualization with local ellipses (Participant 010). **Purple line:** reference trajectory representing the average gait cycle. **Pink ellipses (1σ):** innermost stability zone. **Orange ellipses (2σ):** intermediate stability zone. **Yellow ellipses (3σ):** outer stability boundary. **Black trajectory:** sacrum vertical displacement showing natural gait variability within the ellipsoid torus structure."}
knitr::include_graphics("results/figures/010_state_space_3d.jpg")
```

## Group Comparison: Young versus Aging Adults

Comparison of resilience metrics between **young (n=6)** and **aging (n=2)** groups revealed **systematic differences in recovery dynamics**, particularly for **fast perturbations**.

```{r participant-table, echo=FALSE}
participants_data <- data.frame(
  Participant = c("004", "005", "006", "007", "008", "009", "010", "017"),
  Group = c("Young", "Young", "Aging", "Young", "Young", "Young", "Aging", "Young"),
  Slow_Peak = c(26.42, 24.81, 30.15, 24.81, 28.54, 26.83, 13.92, 40.69),
  Slow_Recovery = c(0.00, 30.32, 18.52, 0.04, 0.10, 0.00, 0.00, 4.46),
  Fast_Peak = c(37.15, 32.36, 39.39, NA, 31.76, 37.94, 14.56, 24.97),
  Fast_Recovery = c(1.22, 6.00, 15.21, NA, 15.32, 0.02, 11.46, 0.04)
)

kable(participants_data,
      col.names = c("Participant", "Group", "Slow Peak (mm)", 
                    "Slow Recovery (s)", "Fast Peak (mm)", "Fast Recovery (s)"),
      caption = "**Table 1. Resilience Metrics for All Participants (N=8). Note: P007 Fast metrics unavailable due to data processing error.**",
      align = c('c', 'c', 'c', 'c', 'c', 'c'),
      digits = 2) %>%
...
```{r peak-comparison, fig.cap="**Figure 6. Peak Deviation: Young vs Aging.** Left: slow perturbations. Right: fast perturbations. Boxplots show median, IQR, extremes. Outlier in young slow (P017: 40.7mm) indicates individual variation.", out.width="100%"}
...
```{r recovery-comparison, fig.cap="**Figure 7. Recovery Time: Young vs Aging.** Left: slow (high variability in young). Right: fast showing clear separation - young median ~1s vs aging ~13s (three-fold difference).", out.width="100%"}
...
Synchronization framework validation used **two technical sessions (P016, P017)** comprising **overground walking** with mat-crossing perturbations. **Cross-correlation analysis** between Codamotion and BeatMove vertical accelerations yielded **near-perfect alignment with R² > 0.999** for both sessions.
...
Dufourneau, A. (2026). *Locomotor resilience analysis: Dataset and MATLAB processing pipeline* [Unpublished raw data and scripts]. EuroMov Digital Health in Motion, University of Montpellier.
